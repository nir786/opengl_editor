...
std::string pathVertCode = Loadfile("/home/nir/editor/shaders/text/text.vert"); 
std::string pathFragCode = Loadfile("/home/nir/editor/shaders/text/text.frag"); 
...
void TextShader::load_stb_atlas(){
  std::vector<unsigned char > fontBuffer = LoadTTF("/home/nir/Downloads/Notedry.ttf");
  stbtt_fontinfo font ; 
   if(!stbtt_InitFont(&font, fontBuffer.data() , stbtt_GetFontOffsetForIndex_internal(fontBuffer.data(), 0))){
...
stbtt_GetFontVMetrics(&font, &ProgramFont.ascent, &ProgramFont.descent, &ProgramFont.lineGap); 

std::cout << "font loaded" << std::endl; 
std::cout << "ascent: "<< ProgramFont.ascent * ProgramFont.scale << std::endl; 
std::cout << "descent: "<< ProgramFont.descent * ProgramFont.scale << std::endl; 
std::cout << "linegap: "<< ProgramFont.lineGap * ProgramFont.scale << std::endl; 
std::cout << "font size"<< ProgramFont.scale <<std::endl;  
FontData.width =512 ; 
FontData.hieght= 512; 
std::vector<unsigned char >atlas(FontData.width* FontData.hieght);  
stbtt_pack_context pc ; 
stbtt_PackBegin(&pc, atlas.data(), FontData.width, FontData.hieght, 0 , 1 , nullptr); 


stbtt_PackFontRange(&pc, fontBuffer.data(), 0 , fontOffsets.size,32, 95 ,FontData.chars ); 
stbtt_PackEnd(&pc); 
...
glTexImage2D(GL_TEXTURE_2D , 0 , GL_RED,
              FontData.width , FontData.hieght, 
...
glBufferData(GL_ARRAY_BUFFER,vertecies.size()*sizeof(vertexData)*6 , vertecies.data() , GL_STATIC_DRAW );//if not drawing * sizeof(float) 
glEnableVertexAttribArray(0);

glVertexAttribPointer(0,4 , GL_FLOAT , GL_FALSE , sizeof(float)*4, (void*)0 ); 

glBindVertexArray(0);
TextShaderData.projection = glm::ortho(0.0f, (float ) Glwindow::get().width ,(float) Glwindow::get().highet , 0.0f , -1.0f ,1.0f); 
...
stbtt_GetPackedQuad(FontData.chars,FontData.width ,FontData.hieght,ch - 32 ,&curLine.x ,&curLine.y ,&q,1 ); 
...
glUniformMatrix4fv(glGetUniformLocation(TextShaderData.program,"projection"),1, GL_FALSE, glm::value_ptr(TextShaderData.projection)); 
glUniform3f(glGetUniformLocation(TextShaderData.program , "textColor"),TextShaderData.color.r ,TextShaderData.color.g , TextShaderData.color.b); 
