...
void Shaders::create_path(float boxWidth , float boxHight , float ceterX , float ceterY){
  vet_path.clear(); 
  ind_path.clear();  
...
std::cout <<left << " left < " << right << "right < " << top << " top < "<< bottom << "bott<< " << std::endl; 
...
std::string pathVertCode = LoadFile("/home/nir/editor/shaders/pathLookUp.vert"); 
std::string pathFragCode = LoadFile("/home/nir/editor/shaders/pathLookUp.frag"); 
...
void Shaders::attach_shader( std::vector<float> vertecies , std::vector<unsigned int> indicies , ShaderInfo * shader ,glm::vec4 color ){
...
glVertexAttribPointer(1,1 , GL_FLOAT , GL_FALSE , 5*sizeof(float), (void*)(sizeof(float)* 4 ) ); 
glEnableVertexAttribArray(1);

glBindBuffer(GL_ELEMENT_ARRAY_BUFFER , ebo ); 
glBufferData(GL_ELEMENT_ARRAY_BUFFER,indicies.size()*sizeof(unsigned int), indicies.data(), GL_STATIC_DRAW); 
glBindVertexArray(0);
  shader->projection = glm::ortho(0.0f, (float ) Glwindow::get().width ,(float) Glwindow::get().highet , 0.0f , -1.0f ,1.0f); 
  ...
  shader->highlightLoc = glGetUniformLocation(widgetShader.program, "outLineColor");  
...
glUniformMatrix4fv(shader->projLoc , 1 , GL_FALSE , glm::value_ptr(shader->projection));
glUniformMatrix4fv(shader->modelLoc , 1 , GL_FALSE , glm::value_ptr(shader->model));
glBindVertexArray(vao); 
glDrawElements(GL_TRIANGLES ,Maneger::get().widget_buffer.size()*6  , GL_UNSIGNED_INT , 0 );
}

void Shaders::init_shaders (){
widget_shader_init( 
"/home/nir/editor/shaders/shader.vert", "/home/nir/editor/shaders/shader.frag") ;
...
ShaderInfo* Shaders::get_widget_shader(){
return &widgetShader; 
} 
...
glBindBuffer(GL_ARRAY_BUFFER , vbo); 
glBufferData(GL_ARRAY_BUFFER,vet_path.size()*sizeof(float) , vet_path.data() , GL_STATIC_DRAW ); 
...
glBindBuffer(GL_ELEMENT_ARRAY_BUFFER , ebo ); 
glBufferData(GL_ELEMENT_ARRAY_BUFFER,ind_path.size()*sizeof(unsigned int), ind_path.data(), GL_STATIC_DRAW); 
glBindVertexArray(0);
  Path_selection.projection = glm::ortho(0.0f, (float ) Glwindow::get().width ,(float) Glwindow::get().highet , 0.0f , -1.0f ,1.0f); 
  Path_selection.model =glm::mat4(1.0f); 
  Path_selection.projLoc = glGetUniformLocation(widgetShader.program, "projection"); 
  Path_selection.modelLoc =glGetUniformLocation(widgetShader.program, "model");  
  Path_selection.colorLoc= glGetUniformLocation(widgetShader.program, "costumColor"); 
  Path_selection.myColor = jason::get().WIDGET_C.WIDGET_BACKROUND ; 
  Path_selection.highlightLoc = glGetUniformLocation(widgetShader.program, "outLineColor");  
...
glUniform4fv(Path_selection.highlightLoc, 1 , glm::value_ptr(Path_selection.highlightcolor)); 
glUniform4fv(Path_selection.colorLoc , 1 , glm::value_ptr(Path_selection.myColor)) ;
glUniformMatrix4fv(Path_selection.projLoc , 1 , GL_FALSE , glm::value_ptr(Path_selection.projection));
glUniformMatrix4fv(Path_selection.modelLoc , 1 , GL_FALSE , glm::value_ptr(Path_selection.model));
glBindVertexArray(VaoPath); 
glDrawElements(GL_TRIANGLES ,Maneger::get().widget_buffer.size()*6  , GL_UNSIGNED_INT , 0 );
  ...
  void Shaders::build_shader_fr_path(const char* vertPath ,const char * fragPath , GLuint vao ,ShaderInfo * program  ){
...
glLinkProgram(program->program); 
  }//creation and compile shadercode into a spasific program make sure you put your disried paths in the init functin
   //make it hot swapable via the json  file 
...
build_shader_fr_path("/home/nir/editor/shaders/toolbar/toolbar.vert", "/home/nir/editor/shaders/toolbar/toolbar.frag", ToolBarVao , &ToolBar_shader); 
...
std::cout << "attached toolbar_shader"<<std::endl; 
  ...
  void Shaders::update_Tool_buffers(float boxWidth, float boxHight , float ceterX ,float  ceterY ){
//also call this one on the init function actually make a semerate shader_init for this type of shader
...
glBindBuffer(GL_ARRAY_BUFFER , vbo); 
glBufferData(GL_ARRAY_BUFFER,vertex_ToolBar.size()*sizeof(float) , vertex_ToolBar.data() , GL_STATIC_DRAW ); 
...
glBindBuffer(GL_ELEMENT_ARRAY_BUFFER , ebo ); 
glBufferData(GL_ELEMENT_ARRAY_BUFFER,ind_ToolBar.size()*sizeof(unsigned int), ind_ToolBar.data(), GL_STATIC_DRAW); 
glBindVertexArray(0);
  ToolBar_shader.projection = glm::ortho(0.0f, (float ) Glwindow::get().width ,(float) Glwindow::get().highet , 0.0f , -1.0f ,1.0f); 
  ToolBar_shader.model =glm::mat4(1.0f); 
  ToolBar_shader.projLoc = glGetUniformLocation(widgetShader.program, "projection"); 
  ToolBar_shader.modelLoc =glGetUniformLocation(widgetShader.program, "model");  
  ToolBar_shader.colorLoc= glGetUniformLocation(widgetShader.program, "costumColor"); 
  ToolBar_shader.myColor = jason::get().WIDGET_C.WIDGET_BACKROUND ; 
  ToolBar_shader.highlightLoc = glGetUniformLocation(widgetShader.program, "outLineColor");  
  ToolBar_shader.highlightcolor = jason::get().WIDGET_C.WIDGET_HIGHLIGHT; 
  }
//its probably stupid to do it that way but i forgot i made a general function to do this also wed need to add parameters later 
//well im fuck i really hadnt thought of this for the futher make sure you add a genral way to add to all the shaders crist this is alot of work 
 void Shaders::render_Toolbar(){
glUseProgram(ToolBar_shader.program) ; 
glUniform4fv(ToolBar_shader.highlightLoc, 1 , glm::value_ptr(ToolBar_shader.highlightcolor)); 
glUniform4fv(ToolBar_shader.colorLoc , 1 , glm::value_ptr(ToolBar_shader.myColor)) ;
glUniformMatrix4fv(ToolBar_shader.projLoc , 1 , GL_FALSE , glm::value_ptr(ToolBar_shader.projection));
glUniformMatrix4fv(ToolBar_shader.modelLoc , 1 , GL_FALSE , glm::value_ptr(ToolBar_shader.model));
glBindVertexArray(ToolBarVao ); 
glDrawElements(GL_TRIANGLES ,Maneger::get().widget_buffer.size()*6  , GL_UNSIGNED_INT , 0 );
 }
