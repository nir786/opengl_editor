...
public:
  // add shaders here
  // make all of these into a vector for hotswap shaders on the file also
  // inherit json
  ...
  void attach_shader(std::vector<float> vertecies,
                     std::vector<unsigned int> indicies, ShaderInfo *s,
                     glm::vec4 color);
  ...
  void init_Toolbar();
  void update_Tool_buffers(float boxWidth, float boxHight, float ceterX,
                           float ceterY);
  void build_shader_fr_path(const char *vertPath, const char *fragPath,
